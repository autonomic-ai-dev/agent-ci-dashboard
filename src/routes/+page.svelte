<script lang="ts">
	import { Activity, Clock, CheckCircle, XCircle, AlertCircle, RefreshCw, Search } from '@lucide/svelte';
	...
	let systemHealthText = $derived(
		failingCount > 0 ? `System Degraded (${failingCount} failing)` :
		pendingCount > 0 ? `CI In Progress (${pendingCount} building)` :
		'All Systems Operational'
	);
	
	let systemHealthClass = $derived(
		failingCount > 0 ? 'bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/20' :
		pendingCount > 0 ? 'bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 border-yellow-500/20' :
...
<div class="max-w-[1200px] mx-auto px-6 py-12 flex flex-col min-h-screen relative z-10">
	...
	<div class={`mb-10 flex items-center justify-center py-4 px-6 rounded-2xl border font-bold text-lg tracking-wide shadow-sm transition-colors ${systemHealthClass}`}>
		{systemHealthText}
	</div>
			...
			{#each filteredStatuses as item, i}
				...
				>
					...
					<div class="flex items-start justify-between mb-4 z-10 relative">
						...
						<!-- Status Pill Minimal -->
						<div class={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold tracking-wide uppercase border bg-surface-light dark:bg-surface-dark ${getStatusColorClasses(item.status)}`}>
							<!-- Glowing Dot -->
							<span class="relative flex h-2 w-2 mr-1">
							  {#if item.status === 'pending'}
								<span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-yellow-400 opacity-75"></span>
							  {/if}
							  <span class={`relative inline-flex rounded-full h-2 w-2 ${getDotColor(item.status)}`} style="box-shadow: 0 0 8px currentColor"></span>
							</span>
							{item.status}
						</div>
					</div>
					...
					<div class="flex-1 flex flex-col justify-center min-h-[80px]">
						{#if item.status === 'no-runs'}
							<p class="text-sm text-text-secondary-light dark:text-text-secondary-dark italic">No workflows ran on this commit.</p>
						{:else if item.status === 'no-tags'}
							<p class="text-sm text-text-secondary-light dark:text-text-secondary-dark italic">No releases found.</p>
						{:else}
							<div class="mt-4 pt-4 border-t border-border-light/50 dark:border-border-dark/50 z-10 relative">
								<div class="flex flex-col gap-2">
									{#each item.workflows.slice(0, 3) as workflow}
										{@const WfIcon = getStatusIcon(workflow.status)}
										<div class="flex items-center justify-between text-[13px]">
											<span class="text-text-secondary-light dark:text-text-secondary-dark truncate pr-4">{workflow.name}</span>
											<div class="flex items-center gap-1.5 shrink-0">
												<WfIcon size={14} class={workflow.status === 'success' ? 'text-green-500 dark:text-green-400' : workflow.status === 'failure' || workflow.status === 'timed_out' ? 'text-red-500 dark:text-red-400' : 'text-yellow-500 dark:text-yellow-400'} />
											</div>
										</div>
									{/each}
								</div>
					...
					<!-- Footer Info -->
					<div class="mt-5 pt-3 border-t border-border-light/20 dark:border-border-dark/20 flex items-center justify-between text-[12px] text-text-secondary-light/70 dark:text-text-secondary-dark/70 relative z-10">
						<div class="flex items-center gap-2">
							{#if item.actor}
								<img src={item.actor.avatarUrl} alt={item.actor.login} class="w-5 h-5 rounded-full opacity-80" />
